...
- 当前模式：标准迭代（进行中）
- 当前阶段：v0.2 **PRD 阶段 — R3 复审中**：Developer 已复审（2026-07-27，**未通过**，3 高 2 中 2 低）；DevOps 已复审（2026-07-27，**未通过**，3 高 2 中）；**Architect 待复审**。两方均确认 R1 意见已全部收敛，新问题集中在 R2/R3 新增的 async 范围与部署环境。
...
- **外部/环境依赖（DevOps R3 订正：原写「唯一外部依赖=口令」不成立，实为两项）**：
  1. `ai_worker` 口令注入（同机部署，从服务器 `/root/.secrets/ai_worker_news_test.pw` 直接读入部署目录 `.env`，无需 Owner 人肉转交、不经对话传递；Owner 只需授权访问）。连接四要素已收：`127.0.0.1:5432` / `news_test` / `ai_worker`。已登记跨任务待办 P0。
  2. **服务器部署环境（新增，DevOps R3 问题 1）**：实查本机为 macOS（`/root` 不存在）、`5432` 无监听、项目内无 `scripts/`·`deploy/`，v0.1 全程跑在本机 `127.0.0.1` —— 即 PRD 说的「与 xiaobao 同机」指一台 **Linux 服务器**，ai 的运行位置要从开发机迁过去，而该环境（代码上机方式 / Python 环境 / `.env` 与日志落点 / 启动命令）**整块不存在**。
  两项均不阻塞设计与实现，但**共同阻塞** C-6 行锁实证与任何联调冒烟；Developer R3 高③已要求把实证前移为实现阶段开工前置，则这两项须一并前移。
...
- **本迭代最大技术风险已转移**：R1 时阻塞全在外部契约缺项（现闭合 9/10），R3 后 ai 侧最大不确定性是 **async 改造的回归**（O-8 标 P0，"风险在回归不在重写"）。Developer 复审指出该风险唯一的客观兜底是「改造前先在 v0.1 基线录制黄金样本快照、改造后逐字段比对」——目前只写在开放问题 O-8 里，**未进 AC-9.4 验收标准**，需 R4 补；否则"行为不变"只能靠改写后的测试自证（循环论证）。
- 下一步入口：① **切 Architect 做 PRD R3 复审**（最后一方；复核 C-3 结论反转对设计的影响。Developer / DevOps 已交，均未通过）；② PM 按三方意见改 **R4**——Developer 侧三条必改：补 HTTP 模式并发退化验收、黄金样本回归基线进 AC-9.4、口令注入前移为实现阶段开工前置（解 AC-3.6 与 §5 的时序冲突）；**DevOps 侧三条必改**：§5 增列「服务器部署环境」为第二项待办并订正「唯一外部依赖」措辞、AC-5.7 补宽限期下限（≥单批最长处理时间）与应用/ASGI/托管三层一致、US-4 与 AC-1 补明回滚是 ai↔xiaobao **双侧协同且有顺序**的动作。**注意 DevOps 高① 与 Developer 高③ 是同一处的两层**——要前移的不只是口令注入，还有整个服务器部署环境（ai 至今只在开发机跑过）；③ **DevOps 备环境 + 注入口令后执行 C-6 行锁实证**（6 步执行安排见 PRD §R3 DevOps 复审，全程事务内 `ROLLBACK`，不消耗 `news_test` 仅有的 5 条预置队列；结论回帖 coordination 归 PM）；④ R4 定稿后进设计阶段：O-2 数据源协议边界、O-6 事务与连接、**O-8 async 改造的切分与回归策略（P0）**、O-9 async DB 驱动选型（Developer 建议 psycopg3(async)）、O-10 `locked_by` 标识规则；④ 实现阶段按 §8 五块切片推进（§8 成本表需按 async 前提重估，见 Developer R3 问题 4）。
...
|------|----------|-----|----|----------|---------|------|
| v0.2 | [v0.2.md](iterations/v0.2.md) | [v0.2-prd.md](iterations/v0.2-prd.md) | 纯后端（无界面） | — | — | 进行中（PRD R3 复审中：Developer / DevOps 已交均未通过，Architect 待做；4 条阻塞全闭，契约 v1.3） |
