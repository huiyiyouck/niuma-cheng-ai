...
- 当前模式：标准迭代（进行中）
- 当前阶段：**部署阶段已完成（生产上线、Demo 版链路跑通，2026-08-03 DevOps）→ 迭代关闭检查已执行（2026-08-03 PM）→ ❌ 不予关闭，卡两条门禁，均须 Owner 处置**
  - **技术侧已全部齐备**：实现阶段**已定稿**（R4 两方通过·无附条件）；**210 passed / 10 skipped**，服务器裸 `pytest` **亦 210**；**部署就绪检查通过**（A~F 六组）；**生产已部署**；真实数据端到端 **7 条全 `succeeded`、零降级、两条 `domain_tags` 路径均覆盖**；**CN-009~011 全部三方确认**，CN-010 九项 **9/9 落地**、CN-011 变更 1/2 落地。
  - **❌ 门禁一：Owner 验收未签** —— `mechanisms.md` 明列「Owner 验收结论已签」为关闭判据，「打回」则禁止关闭（G5 打回闸）。**PM 不得代签**，请 Owner 在 [v0.2.md §迭代关闭归档](iterations/v0.2.md) 首行签署。
  - **❌ 门禁二：`l1-gates` 查不到该 workflow** —— 本仓 `.github/workflows/` **不存在**。`runtime.md` [P0] 把「**无该 workflow**」明文列进非绿情形，**fail-closed**：不自动推进，退回 Owner 确认 + 登记降级留痕（**已登记**）。
    - **PM 建议**：认可人工证据**降级放行**本迭代，把「补 `l1-gates`」排进 v0.3。理由——此时补 CI 不改变任何已验证结论，却要再拖一轮关闭；但**长期必须补**：本迭代六次「校验通过但保证不成立」全靠人工 Review 抓出，**这恰恰说明不该只靠人工**。
  - **⚠️ 非门禁待补（归 Architect，不阻塞关闭判定）**：CN-010 / CN-011 的「设计文档落地」勾选项未勾——**实现已按 CN 落地，缺的是设计文档回填**。PM 不代勾。
