...
- 当前模式：标准迭代（进行中）
- 当前阶段：**部署阶段已完成 —— v0.2 生产部署上线、Demo 版链路已跑通**（2026-08-03）。实现阶段两方 Review 齐·均通过无附条件已定稿；生产 `niuma-ai-worker@prod`（8103 / `news` 库）运行中；**真实数据端到端 7 条全部 `succeeded`、零降级、两条 `domain_tags` 路径均覆盖**；部署就绪检查通过（B 组含事故补入的 B5）。→ **待 Owner 验收 + 迭代关闭检查**；v0.3 主题已定（评分维度细致拆分，Owner 2026-08-08）
  - **Developer 实现 R1 已交付**：按设计 §6.1 步 0~5 共 **13 个提交**（`a9eb235` → `2f5c3c6`），自测报告见 [v0.2-test-report.md](iterations/v0.2-test-report.md)；**联调五项全部完成**（正常解析 / 失败重试 / 卡死回收 / ai 不可用不阻塞 / 双模式切换，含失败重试与卡死回收实测通过）。
  - **Architect Review：通过（附条件）**（1 高 3 中 2 低，无阻塞级）。附条件 = 主循环异常边界须经 Change Note 处置 → 已出 **CN-010**。
  - **DevOps Review：四轮全部交齐**（R1 通过·附条件 1 高 2 中 / R2 通过·附条件 2 中 / R3 通过·无附条件 / R4 通过·无附条件）；**实现阶段两方齐·均通过无附条件·已定稿**。
  - **CN-010 / CN-011 三方确认中** —— **PM 已确认两条（2026-08-02，均通过·无保留）**；待 Developer、DevOps 投票。
  - > **本行此前停留在「设计阶段」直至 2026-08-02 由 PM 订正。** 实现 R1 于同日提交、Architect Review 亦已完成，而项目级真源仍显示设计阶段——**任何人照 INDEX 判断「现在卡在哪」都会判错一整个阶段**。这与本迭代反复出现的问题同型：**不是信息错，是过期时没有信号**。
...
- ~~**联调判读须知**：`score_total` 在 database 模式无触发点、写回后保持 NULL~~ → **✅ 已消除（2026-08-03）**：对方轮询补算已落地生效，生产 7 条成功件 **`score_total` 补算 7/7**。该判读须知**不再适用**。
- 下一步入口：~~① CN-010/011 投票 ② 实现 R2 ③ DevOps 交 R1 Review ④ 部署就绪检查~~ **①~④ 全部完成**（CN-010/011 三方确认齐、实现 R1~R4 四轮定稿、DevOps 四轮 Review 交齐、部署就绪检查通过）→ ⑤ **Owner 验收**（依据：生产真实数据 7 条全部 `succeeded`、零降级、`needs_context`/四维/`score_total` 各 7/7、两条 `domain_tags` 路径均覆盖）→ ⑥ **迭代关闭检查** → ⑦ **v0.3 立项**：主题已定 **评分维度的更细致拆分**（Owner 2026-08-08）。**DevOps 侧无阻塞项。**
  - **PM 侧本阶段无 Review 任务**（实现 R1 指定 Review 方为 Architect、DevOps）；PM 的活是 Change Note 确认、跨项目沟通、以及交付口径与已知限制的收口。
  - **PM 已登记的两条交付口径要求（CN-010 确认）**：① v0.2 对外可用性口径由「一次 DB 抖动即停摆待人工」改为「**能自愈 5 分钟内的 DB 抖动，持续故障才停摆**」，须写进交付说明；② 连续失败期间 worker 仍 `running`、`/health` 仍 200，**外部看不出它在反复失败** → `loop_iteration_failed` 必须带 `consecutive` 计数，且 **DevOps 灰度看护清单须含「查该日志是否出现」**，否则 5 分钟的自愈能力会变成 5 分钟的静默。
  - **PM 已提请 Architect 判断的一条**：`L1_LOOP_FAILURE_LIMIT × 轮询间隔 < 对方 600s 卡死回收` 这个关系**是否值得进启动门禁**——将来有人把 20 调到 60，判死窗口翻到 15 分钟越过 600s，会出现「ai 已装死但未判死、条目被对方反复回收」的窗口，而**不会有任何信号**。
...
| 日期 | 角色 | 工作 | 结论 | 下一步入口 |
|------|------|------|------|------------|
| 2026-08-03~08 | DevOps | **v0.2 生产部署 + Demo 版链路跑通**：新建 `/srv/niuma-ai/prod`（8103 / `news` 库 / 专用用户），部署前核实两库授权逐列对称、队列 0 条；实现 R1~R4 四轮 Review（其中 R1 查出 `deploy.sh` 两处会让下次部署直接中止的缺陷并修完、R2 逐项实测查出「留痕称六项全落地」实为 7/9、R4 认领我提了个不存在的问题）；**一次生产事故换来判据 B5** —— 漏读一条指名归我的待办（08-02 已自报 provider 订阅过期），导致对方放的首批 5 条全废，根因是我那 24 条判据**没有一条验「服务核心能力还在不在」**；修复后真实数据 **7 条全部成功、零降级**、`needs_context`/四维/`score_total` 各 7/7 | 部署就绪检查**通过**；Demo 版达成。**判据的盲区就是执行者的盲区** | 待 Owner 验收 → 迭代关闭检查；v0.3 做评分维度细致拆分 |
