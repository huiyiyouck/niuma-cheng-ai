...
## 2026-08-03 — 收尾 + l1-gates 门禁预检（Owner 选「先补 CI 再关闭」）

- 本次角色：Developer（收尾铺写 + 门禁预检）
- 动作：重写并跑通验收脚本 → 核实部署现状 → 查 `l1-gates` 归属与预跑 → 规范一处状态词
- 结论：**Developer 侧无欠账**；补 CI **不归 ai 项目任何角色**，归参谋长（生态根）。
  1. **`l1-gates` 是框架资产，只能经 `sync-downstream.sh --enable-l1-gates` 首装**（分发 `.github/workflows/l1-gates.yml` + `scripts/gates/l1-gates.sh`）。根 CLAUDE.md 明令「对下游只准经 `sync-downstream.sh` 同步框架文件，**禁止手改下游任何文件**」——我手写一个 workflow 会被回流脚本的 offending 检查判为违规文件，且下次回流直接覆盖。**查清归属再动手，这次没有再自作主张。**
  2. **预跑门禁：当前仓退出码 0，但这个绿灯不检查 v0.2**——`v0.2.md` 不含「## 阶段执行记录」小节，脚本第 23 行直接 `continue`。**必须在关闭留痕里写清楚这一点**，否则下一个人会以为 v0.2 被 CI 验过。用 `L1_REPLAY=1` 强制回放会报大量 G1，但脚本自己声明该模式「不用于执法」，且适用范围写明「历史记录跳过（BCR-011：执法不回溯）」——**这是框架有意划的界，不是漏洞，不该夸大成「绿灯是假的」**。
  3. **门禁从 v0.3 起真正生效**（U1 模板起的迭代记录才含「阶段执行记录」）。故「补 CI」这个选择的实际收益不在 v0.2，而在**此后每次提交都被自动核对**——这恰恰是我在关闭建议里说的那句：本迭代六次「校验通过但保证不成立」全靠人工 Review 抓出来，正说明不该只靠人工。
  4. **规范一处我自己写的状态词**：R4 阶段状态我写成「两方齐 · 均通过（无附条件）· 实现阶段定稿（…）」，**不在封闭枚举**（`待Review/Review中/修改中/已定稿/阻塞/已跳过`，BCR-011）。虽然 v0.2 不受执法，但写法本身不合基线，已改为「已定稿」——详细结论本就在「Review 结果」列，那一格只该放状态词。
  5. **验收脚本的一处自我修正**：最初临时借 openclaw 的 key 跑，能出漂亮结果——但那样验的是「代码行不行」，验不到「部署配置对不对」，**正是「验证者绕开被验证者」，与 R3/R4 栽的两次同型**。改用部署 `.env` 后，配置未修时第【1】项如实报红。
  6. **核实部署时差点报一个假问题**：先查 `test` 环境（`RUN_MODE=http`、worker 未启用），几乎要报「部署没切 db 模式」——实际生产在 `prod` 目录，`RUN_MODE=db`、worker 正常运行。**查错了对象，结论就完全反了。**
- 关联迭代：v0.2
- 遗留问题/风险：① **补 CI 待参谋长执行**（生态根跑 `sync-downstream.sh ../niuma-cheng-ai --enable-l1-gates`）② 两套环境均只配 1 个 provider，ADR-0002 的 fallback 链仍未真正生效，而 B5 判据是用一次生产事故换来的（归 DevOps）③ CN-010/011 的「设计文档落地」勾选（归 Architect）④ AC-2.4 追认 CN（PM/Architect）
- 下一步入口：参谋长回流装 CI → 确认 `l1-gates` 绿灯 → PM 关闭迭代
- 收尾状态：已收尾

## 2026-08-03 — 实现阶段定稿（R4 两方均通过·无附条件）

- 本次角色：Developer（产出方定稿）
- 动作：读两方 R4 Review → 按基线把阶段置「已定稿」→ 落 D-5 已知行为 → 报告收口
- 结论：**实现阶段定稿**（`a9eb235..7649b88`，四轮）。两方均无附条件通过；**210 passed**，部署机裸 `pytest` 亦 210，真实 PG 集成 10 passed。
  1. **定稿是产出方的动作，这次我没再推给别人**。基线 §Review 计划写明「本轮指定 Review 方全部通过 → 产出方将阶段改为已定稿」。R1/R2 我连着两轮把「指定 Review 方」误当成 PM 的活，被 Owner 当场纠正；定稿这一步是同一条规则的下半句，不能再错第二次。顺带把 Architect 落在「阶段状态」列的 Review 意见移回「Review 结果」列（文字一字未改，只归位）。
  2. **变更 9 的前提两方都认领了**：DevOps 认「那个不存在的问题是我 R1 提的，且我 R2/R3 两轮都在验落地、从没验前提」；Architect 认「前提错在我，我核了 `psycopg_pool.close()` 的源码，却从没核过 `asyncio.wait_for` 的——订正了下游机理，没质疑上游前提」。其点出这与 `AI_STALE_TIMEOUT_MS` 的 1800s **完全同型，只不过这次那个「对方」是标准库**。
  3. **我的判据修正被采纳并回写设计 §8**：Architect R3 给的「删掉 `await gather` 后测试必须变红」被证明是错的——保证有两条独立来源，删任一条另一条仍兜着，同时删才破坏。正确形式是「**注入必须破坏保证**」，而非「删掉某一条实现路径」。其明确写「按我原判据这组测试会被误判为无鉴别力，而它实际上比 R3 那版强得多」。
  4. **Architect 特别点出的一条我自己没意识到**：`test_invariant_would_break_if_wait_were_shielded` 把「`wait_for` 的取消语义」这个外部前提写成了断言，**语义一变该测试就红**——本迭代反复吃亏的形状是「被依赖的输入不被任何机制保证」，`AI_STALE_TIMEOUT_MS` 的处置是升格为契约参数，这里的处置是把它写成断言。**同一条思路第一次用在标准库语义上。**
  5. **D-5 按 Architect 判为「v0.2 不改，登记为已知行为」**：停机时 worker 的死亡异常会被重抛一次，让 uvicorn 的 shutdown 日志出现一个像新故障、实为几分钟前那次判死回声的堆栈。修它要在停机路径加分支，而**停机路径是最不该增加分支的地方**（AC-5.7 整条链就在那几行，R1~R4 已在它上面出过三次问题）。已入缺陷表并请 DevOps 记入灰度看护清单。
  6. **四轮里我被纠正的三次，性质是递进的**：R2 代码有正确性缺陷 → R3 验证方法只验了一半 → R4 反向注入的注入点打在复刻件上。**每一次都是「判据答的不是真正要问的那个问题」，只是位置越来越靠近判据本身。**
- 关联迭代：v0.2
- 遗留问题/风险：① CN-010 的「设计文档落地」执行状态未勾（归 Architect）② AC-2.4 追认 CN 待 PM/Architect ③ 部署 LLM 配置待 DevOps（P0）④ C1/C4 判据扩写（DevOps 已认领，部署阶段）⑤ D-5 已知行为须入看护清单 ⑥ 展示层核对属呈现层验收
- 下一步入口：**Owner 验收** → 部署就绪检查（A~F 六组 24 条判据，DevOps）→ 迭代关闭检查
- 收尾状态：已收尾
