...
## 2026-08-02（续 3）— 处置 DevOps 实现 R1 Review：两条我读过那段代码却没看出来

- 本次角色：Architect（架构师）
- 动作：DevOps 交实现 R1 Review（通过·附条件，1 高 2 中）。高① 是其 `deploy.sh` 缺陷、已自行修完；**中②③ 都改设计，收进 CN-010 变更 8、9**。**两方 Review 齐，实现 R1 进入「待附条件落地后定稿」。**

### 一、中②：我拿一条代码里并不存在的链，去论证「正确性不受影响」

PRD AC-4.7 要求四元链 `connect < lock < statement < tx`，实现拆成了两条：三元链 + `connect` 只比 `tx`。**`connect` 与 `lock`/`statement` 之间没有任何约束**，其实测 `connect=4500` 门禁放行（此时 `connect > statement > lock`）。

**这两行 if 我 Review 时读了。** 我读完的判断是「不变式 3 ✓」，还在 CN-010 中② 里写下「`connect`(实际 2s) `< lock`(3s) 仍成立」——**我脑子里拼的是 PRD 上那条链，然后拿它去论证代码的正确性**。结论侥幸没错，但**它成立的原因是默认取值恰好，不是门禁在守**。

### 二、中③：处置采纳，但**机理要订正，而且必须订正**

其称「最坏是事务在 COMMIT 前连接被抽走」。查 `psycopg_pool` 源码与 docstring：**「Currently used connections will not be closed until returned to the pool」**——`close()` 只关池内空闲连接，使用中的不会被抽走；其 `timeout` 等的是池内部维护 task。

真正的机理是：**`lifespan` 的 `finally` 走完 → 进程退出，而 worker 协程的取消尚未完成**。残留锁风险是真的，根因不是池抽连接。

**为什么不能「结论对就行」**：按其机理，一个自然的修法是「给 `pool.close()` 加大 timeout」——**完全无效，因为那个 timeout 根本不等使用中的连接**，而它同样会「测试通过」。**机理错会导出一个看起来合理、实际不解决问题、且验证不出来的修法。**

### 三、这轮三方各自独立踩到同一件事

| 谁 | 表现 |
|---|---|
| 我（中②③） | 代码**存在** ≠ 它做的是我以为的那件事。我核了「有没有」，没核「是不是我以为的那个」 |
| 我（低⑤，Developer 指出） | 用「全仓零引用」回答「它该不该存在」 |
| 我（低③，Developer 指出） | 测试用例能跑 ≠ 有鉴别力 |
| DevOps（其自述） | `/health` 用 `def` 那条**首次实测串行发请求得出相反结论**，改并发才对——**它把错误的中间结论一并写进了 Review** |

**共同形状：判据答的不是我真正要问的那个问题。** DevOps 那条尤其值得记——留错误的中间结论比只留正确终态更有价值，与我 08-01「正则太宽差点误判队友」同源。

### 四、编号冲突

DevOps 把其中② 记为「第七次校验通过但保证不成立」，而我在变更 3 已把「判死窗口漏算 TX」记为第七次。**按发现先后应为第七、第八次**，已在 CN-010 统一，不各记各的。

- 关联迭代：v0.2（**实现 R1 两方 Review 齐·均通过附条件**；附条件全部收敛为 CN-010 九项 + CN-011 一项，两 CN 三方确认齐）
- 遗留问题/风险：① 变更 6 判死方向与 Developer 倾向相反、`LIMIT` 20→15 需 DevOps 与 PM 复核、变更 8/9 为其投票后新增——**三项均已在 CN 内标明可要求重投** ② DevOps 建议 Developer 给 `pytest.ini` 加 `pythonpath = .`（其产出物，未代改）
- 下一步入口：Developer 出实现 R2（落 CN-010 九项 + CN-011 一项）→ 我与 DevOps 复审 → 部署就绪检查

## 2026-08-02（续 2）— 处置 Developer 末票：三条全采纳，两条打在我这个 CN 自己身上

- 本次角色：Architect（架构师）
- 动作：Developer 投出 CN-010/011 末票（两个通过·附条件），三条附条件全部成立、全部采纳，落为变更 6、7 与两处订正。**其技术断言我逐条复算过，无一条不实。**

### 一、CN-010 中①：我修好了「过于敏感」，却把另一条路径改成了「完全不敏感」

变更 1 把写回耗尽后的 `raise` 改为 return（方向对，那是设计原意）。但它与「任一轮成功即清零」叠加后有组合效应：**claim 成功清零，写回失败不抛异常、不进 except、根本不计数**。DB「可达但写不进」时，worker 永远 `running`、`/health` 200、计数**恒为 0**，而条目逐个走完 `attempt` 进 `final_failed`——**整个队列静默报废**。

其对比一针见血：**改前粗暴但有 503 这个信号，改后一个信号都没有。** 而变更 4 那个专为暴露「活着但在挣扎」而加的字段，在这个场景下显示的恰恰是「一切正常」。

**分开计数采纳；但判死方向我定得与其相反**——其倾向不判死（怕牺牲本可正常处理的条目），我判**要判死**：环境性写回故障下没有任何条目能处理完，「本可正常处理的其他条目」不存在；而继续跑是**持续把可恢复的条目烧成 `final_failed` 这个终态**，判死反而让条目留在 `running` 等对方回收→重新排队。**不判死换来的不是可用性，是把可恢复损失变成不可恢复损失。**

**其顾虑在数据相关故障下完全成立，而顺着它我查出一处自己 Review 时漏报的实现偏离**：设计 §4.6 早写明「仅对可重试的 PG 错误类重试，确定性错误不重试」，而 `_commit_with_retry` 是 `except Exception` 无差别重试。补上这条，其顾虑自动消解——确定性错误不重试、不计数、不判死。

阈值取 3 而非像 claim 那样 15：**单次代价差两个数量级**（claim 失败 = 20s 空转；写回失败 = 240s 算力 + 一次 LLM 费用 + 烧一条 `attempt`）。

### 二、CN-010 中②：`error_kind` 硬编码，且它订正了我 Review 的低⑤

其核出穿透主循环的四类异常里有两类与 DB 无关（`run_task` 内部 bug、`get_adapter` 未注册类型）。危害它说得准：**会说谎的日志比没有日志更费时间**；更糟的是**15 条毒数据就能判死 worker，而 `dead_reason` 写 DB 故障，排查从第一步就走错方向**。

**它建议用我判为「死代码」的那个 `_error_kind_of` 做分流——我的低⑤ 判断是错的**。它不是死代码，是**写了但还没接上的分类器**。两者处置方向相反：死代码该删，未接上的该接上。**我当初的判据只是「全仓零引用」——零引用能证明它现在没用，不能证明它不该有用。**

### 三、CN-010 低③ 与 CN-011 中①：我在修一个形状的过程中，又造了同一个形状

- **低③**：我把 `//` 换成 `round` 毫无必要，`round(3500)` 得 4——**配 3.5s 生效 4s，比配置意图更长**，对超时量方向是反的。更狠的是它接着说：**我选的四个测试值（500/1000/1500/3000）在两种实现下结果逐个相同、对此零鉴别力**。我在变更 3 里刚写完「能自动执行的约束才会被验算」，转头写了个**能自动执行但选不出差异**的用例。**可执行 ≠ 有鉴别力。**
- **CN-011 中①**：我的判据 `not isinstance(..., dict)` 只抓「维度对象缺失」，而 `_clamp_score` 的兜底面更宽——有对象无 `score` 键 / 类型非法 / `null` 三种同样产生伪 0 而**判据全放行**，且**这三种比整体缺失更可能发生**（LLM 结构骨架通常是对的，错的是值）。**即我的判据恰好只抓住了最不可能的那一种。** 根因它说得比我准：**我在 `_clamp_score` 之外重造了一套「什么算有效 score」，两套必须永远同步而任一方改动都没有信号**——正是本 CN 理由 2 反对的那种依赖，我在修它的过程中又造了一个。已采纳其 `(score, ok)` 方案。

**三条串起来是同一个毛病**：判断「够不够」时，我问的是「能不能覆盖我想到的那个场景」，而不是「产生这个信息的地方到底有几种出口」。**前者的答案取决于我想象力的边界，后者可以去代码里数。**

### 四、顺带订正 CN-011 一处自相矛盾

其「范围纪律」原写「**不改 `_clamp_score` 的默认值行为**」，而重写后的变更 1 恰恰要改它的签名。已订正为「不改**对外行为**（兜底仍为 0）」——签名改属内部重构，两者不是一回事，原措辞把内部实现也圈了进去。

- 关联迭代：v0.2（两个 CN 三方票齐；实现 R1 的 DevOps Review 仍待交）
- 遗留问题/风险：① **变更 6 的判死方向与 Developer 倾向相反，需其复核**（已在 CN 里标为可争议，若其认为「确定性错误不计数」不足以覆盖其担心的场景可直接驳回）② `LIMIT` 20→15 仍需 DevOps、PM 复核 ③ 变更 6/7 与 floor 订正均为三方投票后新增，已标明任一方可要求重投
- 下一步入口：等 DevOps 交实现 R1 Review → 两个 CN 落设计（§2.6/§3.6/§4.2/§4.4/§4.7/§8）→ Developer 出实现 R2
