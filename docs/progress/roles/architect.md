...
## 2026-08-03（续 2）— 实现 R4 Review：通过。变更 9 修的是一个不存在的问题，而前提错在我

- 本次角色：Architect（架构师）
- 动作：复审实现 R4（`9a57366..7649b88`）。**通过（无附条件）**。R3 附条件已落地，且它顺带推翻了**我定的**变更 9 的前提。

### 一、我实测确证：变更 9 要修的问题在生产代码里本就不存在

```text
裸 wait_for(task)          → TimeoutError 抛出时 task.done()=True   收尾已跑完
包 wait_for(shield(task))  → TimeoutError 抛出时 task.done()=False  还没收到 cancel
```

`asyncio.wait_for` 超时时**已经取消 task 并等它结束**（`_cancel_and_wait`），生产代码用的正是裸 `wait_for(task)`。**R3 那个复刻件用 `shield` 人为制造了这个问题，然后"验证"了修复有效——验的是一个在生产代码里不成立的命题。**

**这是同一个错误的第二层，全在我身上**：R1 中③ 时我订正了 DevOps 的机理（「不是池抽走使用中的连接」——那个订正本身是对的），当时还写下「机理错会导出看起来合理却无效的修法」。**我核了 `psycopg_pool.close()` 的源码，却从没核过 `asyncio.wait_for` 的。订正了下游机理，没质疑上游前提。**

而我在 CN-010 §B 里亲手写过「**新量入门禁前必须验其生效语义**」。**`wait_for` 的取消语义正是这样一个从未被验证过的外部前提**，与 `AI_STALE_TIMEOUT_MS` 的 1800s 同型——**只不过这次那个「对方」是标准库。**

**责任链要看全**：DevOps R1 提出（前提错）→ 我采纳并订正机理（上游没查）→ 三方确认（无人验证前提）→ Developer R3 实现 + 写复刻件测试 → **我 R3 用反向注入发现测试无鉴别力，却仍然没质疑前提，还给了个错判据** → Developer R4 落地附条件时才查出来。**四轮、四个角色、三次 Review，没有一个人去验那个前提**，而我们整个迭代都在说「被依赖的输入要被机制保证」。

保留那两行的决定我确认——从「修复真实缺陷」降格为「对标准库语义变化的防御」，无代价。前提描述已回填 CN-010。

### 二、它的判据修正成立，而被修正的正是我刚写进设计 §8 的那条

我 R3 写的是「删掉生产代码里的 `await gather` 后测试必须变红」。**错在把「保证」等同于「某一行实现」。** 四种注入我独立复现，与其报告逐条一致：

| 注入（全打在生产代码上） | 结果 |
|---|---|
| A 删 `cancel + gather` | 6 passed（`wait_for` 内建取消兜着） |
| B 给 `wait_for` 包 `shield` | 6 passed（A 那两行兜着） |
| **A + B 同时** | **1 failed** ✅ |
| C 删 `pool.close()` | **4 failed** ✅ |

**按我原判据，这组测试会被误判为无鉴别力——而它实际上比 R3 那版强得多。** 正确形式是「**注入必须破坏保证**」。已回写设计 §8 第 4 条，并注明：第 1 条防的是「坏测试伪装成已验证」，**第 4 条防的是「好测试被判成坏的」**。

### 三、最该记住的一条：它把标准库语义写成了会报警的断言

`test_invariant_would_break_if_wait_were_shielded` 断言 `shield` 下 `task.done() is False`，注明「若此断言红了说明 `wait_for` 语义已变」。**把我们依赖的外部前提变成了 Python 升级时会主动报警的东西。**

本迭代反复吃亏于「被依赖的输入不被任何机制保证」：`AI_STALE_TIMEOUT_MS` 的处置是升格为契约参数、`connect_timeout` 的处置是暴露生效值，**这里的处置最通用——写成断言**。已作为附则写进设计 §8。

### 四、观察项的判断（其未擅自改，交我判）

worker 异常结束时 `wait_for` 抛的是该异常本身，会穿过 `except` 直达调用方。不变量仍成立（`finally` 照关池、task 已结束），**其判断正确、不构成缺陷**。问题只在可观测性——`_on_worker_done` 已记过一次判死，停机再抛一次，日志里像个新故障。

**我判 v0.2 不改**：修它要在停机路径加「识别 worker 早已死亡」的分支，而**停机路径是最不该加分支的地方**——AC-5.7 整条链就在这几行里，R1~R4 已经在它上面出过三次问题。用一个可观测性瑕疵换停机路径不再变动，划算。**但须让 DevOps 知道**，灰度期看到该堆栈不应按新故障处置。

### 五、我自己这轮的一次险些误判

读生产代码时我一度以为 `shutdown_worker` 里有一行 `await pool.close()  # <<< 注入` 的残留没还原，**几乎要发出「生产代码被注入污染并已 push」这种最重的指控**。核实后是我复述时把测试文件的注入描述混进了生产代码——`git status` 干净、工作区与 HEAD 逐字一致、`grep 注入` 零命中。

**救我的是「先确认这是 commit 里的还是本地的」这一步。本迭代第二次因为核实而免于误判队友**（上次是 08-01 正则太宽），**而这次的指控严重得多**。

- 关联迭代：v0.2（实现 R4 两方齐 · 均通过无附条件）
- 遗留问题/风险：① 异常回声属已知行为，须进 DevOps 灰度看护清单 ② CN-010 的「设计文档落地」与 Change Notes 表登记两项仍未勾
- 下一步入口：实现阶段定稿 → 部署就绪检查

## 2026-08-03（续）— 实现 R3 Review：通过（附条件）。反向注入救了这一轮，而它也差点骗过我

- 本次角色：Architect（架构师）
- 动作：复审实现 R3（`e509735..9a57366`）。**本轮我不读留痕、逐项实测，并对每项修复做反向注入**——把修好的那行改回坏的，看测试红不红。**结论通过（附条件），1 中。**

### 一、R2 三项附条件：生产代码全部修好，我逐项独立实测

- **白名单**：构造真实 `psycopg.errors` **双向**验 29 个码（14 应可重试 / 15 应不可重试）→ **误判 0**。改按 Class 是对的落法，且它主动写了一个测试论证「按 Class 会过度覆盖为何仍可接受」——**两种误判代价严重不对称**。主动记录并论证过度覆盖，比悄悄收窄边界好得多。
- **四元链**：`connect` 取 4500/3500/2500/1000 四档，比对「链条成立」与「门禁放行」→ **四档全部一致**。
- **`pytest.ini`**：**裸跑（不带 `PYTHONPATH`，即我上轮失败的那个方式）→ 208 passed**。我上轮报的中② 修好了，且它接受了我指出的「DevOps 建议只解一半」，落为 `pythonpath = . src`。

### 二、中①：反向注入是本轮唯一发现的来源，而它差点也骗过我

三次注入：白名单退回 R2 版 → **12 failed** ✅；四元链退回三元链 → **2 failed** ✅；**删 `main.py` 的 `await gather` → 全仓 208 passed，无一变红** ❌。

根因：`test_shutdown_path.py` 的 `_run_shutdown` **复刻**了 `lifespan` 停机段，**包括自己写的那句 `await gather`**。我删的是生产代码那句，复刻件原封不动。全仓 `grep lifespan` 确认：**没有任何测试调用真实停机路径。**

**更深的一层**：R3 门禁行写着「`cancel` 后不等则关池顺序反转 2 failed」——**说明 Developer 做了反向注入，方法它是懂的**。我复现了两种注入：删复刻件那行 → **2 failed**（它报的就是这个数）；删生产代码那行 → **4 passed**。**方法对、执行了、结论反了，因为注入点打在了自己的复制品上。**

**我那一份**：CN-010 测试 37 的判据是我写的——「用调用顺序记录，不用睡眠时长断言」。**它每一条都照做且做对**，验的仍是复刻件。**我约束了怎么断言，没约束断言谁。**

### 三、把反向注入写进设计 §8，且必须写死注入点

本轮三次注入两红一绿——**不做注入我就会判通过，变更 9 的零回归保护会一直留着**。成本以秒计，却是唯一能证明鉴别力的方法。

但只写「要做反向注入」不够：**这条判据本身也会以「已执行」的面貌通过**，正如本轮。故设计 §8 写死三条硬要求，第一条就是**注入点必须落在生产代码上**。

**这是本迭代第三次「验证者与被验证者没有独立性」**（AC-9.4 循环论证 / `assert_read_committed` 验自己刚 SET 的值 / 本次复刻件）。**三次都在事后才发现——问题不在认识，在没把它变成检查项。** 现在成了检查项。

### 四、判通过而非未通过的理由

与 R2 的区别是**性质**：R2 是生产代码有正确性缺陷（数据进不可恢复终态）；本轮生产代码三项全部正确、经我独立实测，唯一问题是一组测试回归保护为零，不影响交付物正确性。

- 关联迭代：v0.2（实现 R3 两方齐 · 通过；DevOps 无附条件、Architect 附 1 中）
- 遗留问题/风险：① 停机段提取 + 测试改为调生产代码，**部署阶段开工前落地** ② CN-010 的「设计文档落地」与 Change Notes 表登记两项仍未勾（DevOps 也点了这条）
- 下一步入口：附条件落地 → 实现阶段定稿 → 部署就绪检查

## 2026-08-03 — 实现 R2 Review：未通过。我给的白名单少了 5 类，而漏掉的正是本 CN 立论的那个场景

- 本次角色：Architect（架构师）
- 动作：复审实现 R2（`2f5c3c6..e509735`）。**结论未通过**，1 高；另认可 DevOps 已逐项实测出的变更 8、9 未落地。

### 一、高①：`classify_error` 白名单漏 5 类，PG 例行重启被判成「这条数据自己有问题」

实测构造真实 `psycopg.errors` 调分类函数：`57P01`(admin_shutdown) / `57P02`(crash_shutdown) / `57P03`(cannot_connect_now) / `53300`(too_many_connections) / `55P03`(lock_not_available) **全部被判为不可重试**。后果链逐段查过：

```text
(db_error, False) → gave_up_deterministic：一次重试都没有
  → mark_failed(retryable=False) → final=True → l1_status='final_failed'（终态）
  → 且不计入 consecutive_writeback_failures → 不判死、不告警
```

**PG 例行重启期间正在写回的条目，被当成脏数据直接烧成终态**，而它完全正常、只是撞上重启，且已花掉 240s 预算和一次 LLM 调用。

**两处讽刺要记下来**：① 这与我在 CN-010 变更 6 写的「不判死是把可恢复损失变成不可恢复损失」直接冲突——**这里比不判死更糟，一步到终态**；② **变更 1 从头到尾的立论就是「PG 例行重启会走到这条路径」**，整个 CN 的动机之一就是它，结果错误分类恰恰把它判成不可重试。

**根因在我的设计 §4.6**：清单只写了「连接错误 / deadlock / serialization」三类。**Developer 忠实照做还多补了 `08*` 与无 sqlstate 的 `OperationalError`——它做得比我的清单多，缺口仍在我这。** 其白名单制方向对（黑名单会让新错误默认进重试），**但粒度错了：按个别码列，漏的永远是没想到的那个**。建议按 Class 给：`08*`/`53*`/`57*` + `40001`/`40P01`/`55P03`。

**它的实测为什么没抓到**：其验的五项全是「验该判 False 的判没判对」，**没有一项验「该判 True 的有没有被误判成 False」**。白名单的风险从来不在名单内。**第九次同型。**

### 二、DevOps 比我更彻底，而我该认领它没点破的那一半

它**不读留痕、逐项实测九项变更**，做出完整验收表，独立查出变更 8、9 未落地（我在其发布前也测到 `connect=4500` 仍放行，但没做成逐项表）。它还区分出「**投票射程**」与「**实现射程**」——我在 CN-010 里只意识到前者。

**但根因要说得更直白**：变更 8、9 是**我在三方票齐之后追加进 CN 的**。我知道有风险（写了提醒），却**只提醒了投票方、没提醒落地方**，也没让「CN 有几项变更」变成任何可核对的东西。**「六项」忠实反映了它的实现依据——数字没错，错在我让 CN 变成了移动目标。**

### 三、落地部分有三处强于我给的方案

1. **`classify_error` 提为 `PullSource` 协议方法** —— 我在变更 7 只写「用 `_error_kind_of` 分流」，**照我写的做 worker 就得 `import psycopg`，AC-2.2 当场破**。它识别出这个矛盾并把翻译点下沉到数据源层。
2. **判死检查放 `try` 之前** —— 两个"自然"位置都是错的（放 `except` 会被自己捕获，放轮末会被空队列 `continue` 绕过），我在 CN 里没写这个细节。
3. **反向验证** —— 把两个计数合并跑一遍证伪自己的实现。**这是我要求「测试要有鉴别力」之后第一次有人主动反向证伪。**

CN-011 落地忠实，且它**解决了我 CN 里一处自相矛盾**（明细进 `degradations` 等于进标记）——矛盾是我造的。

### 四、我自己的两处订正

1. **实现 R1 Review 我写「PM 未另行指定」，这个流程理解是错的** —— 基线明确指定 Review 方是**产出方**的活。Developer 查基线纠正了自己连犯两轮的错，**同时也纠正了我**：我代它填那一格时用的是同一个错误理解。
2. **我一度把「本机 174 passed」当验证证据摆出来** —— 我的 venv 里 `agent_hub` 根本没装，是 `PYTHONPATH=src` 凑的。**Owner 当场问「你在哪里跑通的」，问得对。**

### 五、为什么我判未通过而 DevOps 判通过（附条件）

**性质不同**：其两条是「已确认的变更没做」，补上即可；**我这条是已落地部分的正确性缺陷**，后果是数据进不可恢复终态。三项一起，R2 不该定稿。R3 工作量很小。

- 关联迭代：v0.2（实现 R2 **未通过**，需 R3）
- 遗留问题/风险：① 设计 §4.6 的可重试清单归我补全，**须在 R3 开工前落地**，否则 Developer 没有正确依据 ② `pytest.ini` 缺 `pythonpath = . src`，三人三种跑法各自都绿（DevOps R1 建议只解了一半）③ R2 新增 32 个测试未在服务器进程内跑过（集成测试连真实 PG 但进程在本机）
- 下一步入口：**我先补设计 §4.6 清单** → Developer 出 R3（白名单按 Class + 变更 8/9 + 三个测试）→ 我与 DevOps 复验
