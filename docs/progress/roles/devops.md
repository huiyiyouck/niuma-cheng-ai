...
- 遗留问题/风险：① **`N = 1` 在 600s 阈值下是唯一合法值**（`N=2` 即违反），单条预算 240s 仅剩约 97s 上调空间——灰度期若嫌不够，**调整前必须重核不变式**，这条已写进部署就绪检查 B1 ② `dead` 状态在 v0.2 无自动消费方 + 「unit 存在 ≠ 托管化完成」→ **灰度期只能人工看护**（F4）③ D1 的 KB 同机前提须在部署时显式核对（迁机则 KB 全失败但主流程不中断、只持续降级）④ 6i② 待 xiaobao 补 `domain_tags` 非空样本，否则「有值」路径直到生产才首次执行 ⑤ `niuma-ai-worker@test` 待 worker 代码就绪后 enable ⑥ ~~v0.1 服务（8100）处置待 Owner 定~~ **已停机（见结论⑦）**；**遗留一条待 Owner 定**：是否就「xiaobao prod 配置仍指向已停的 8100」经 coordination 知会对方（建议知会——其 prod 一旦启用 L1 HTTP 调用会连接失败，且失败点在对方侧、排查成本高）。
